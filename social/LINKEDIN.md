...
1. **Klein's pre-mortem** (Harvard Business Review, 1998 / 2007). Before any code is written, the agent is forced to imagine the project has failed and enumerate concrete causes. Klein's empirical work showed prospective hindsight identifies approximately 30% more risks than naive forward-looking analysis. ASP runs this as Phase 2 step 1.
...
4. **Reflexion-style validator with prompt isolation** (Shinn et al., 2023, plus EMNLP 2025 follow-up work on self-critique limits). The validator subagent receives only the planner's emitted artefacts (charter, macro plan, deliverables register, shadow-point list) — never the planner's reasoning trail. Same-context self-critique fails on high-confidence hallucinations; prompt isolation breaks that collusion. We codify this as Iron Law 2: NO PLAN APPROVAL WITHOUT INDEPENDENT VALIDATOR PASS.
